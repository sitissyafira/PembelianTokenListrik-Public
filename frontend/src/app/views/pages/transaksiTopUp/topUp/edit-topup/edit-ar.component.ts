...
export class EditArComponent implements OnInit, OnDestroy {
				...
				this.serviceTopUp.getDetailTransaksi(id).subscribe(res => {
					if (res) {
						...
						const datas = {
							...
							adminRate, nameUnit: unit,
							totalBiaya, companyBank,
						...
						if (res.data.engineer !== null) {
							...
							console.log(res.dataEngineer._id);
	...
	createForm() {
		this.arForm = this.arFB.group({
			...
			card_no: [{ value: "", disable: false }],
			name_card: [{ value: "", disable: false }]
		});
	}
	...
	*/
	...
	cekVAPower() {
		if (this.topup.mtdPembayaran === 'edc' || this.topup.mtdPembayaran === 'cash') {
			return false
		} else if (this.topup.mtdPembayaran === 'manual') {
			if (this.topup.companyBank === null || this.topup.companyBank === '') {
				return true
			}
		}
	}
	...
	*/
	...
	getValueToken(e: any) {
		...
		this.EngineerResultFiltered = undefined
		this.valueIdEngineer = undefined
	...
	valueNoCard: string = ""
	valueNameCard: string = ""

	getValueNoCard(e: any) {
		this.valueNoCard = e.target.value
	}

	getValueNameCard(e: any) {
		this.valueNameCard = e.target.value
	}
	...
	prepareAr() {
		...
		const statusDonePrgr = "done"
		if (this.valueNoToken !== undefined && this.valueNoToken.length === 20) {
			formData.append("prgrTransaksi", statusDonePrgr);
			formData.append("noToken", this.valueNoToken);
			console.log("no token sudah valid ");
		} else if (this.valueNoToken === undefined && this.valueIdEngineer === undefined) {
			console.log("no token tidak valid ");
			return false
		} else if (this.valueNoToken !== undefined && this.valueNoToken.length !== 20) {
			return false
		}
		...
		if (this.valueFiles[0] !== undefined) {
			formData.append("proof_of_payment", this.valueFiles[0]);
		} else if (this.valueFilesEngineer[0] !== undefined) {
		...
		console.log(this.valuePrgrTransaksi);

		formData.append("statusPayment", this.valuePaid);
		formData.append("card_no", this.valueNoCard);
		formData.append("name_card", this.valueNameCard);
