...
export class EditPackagesComponent implements OnInit, OnDestroy {
	...
	onSubmit(withBack: boolean = false, validate) {
		...
		if (this.packages.package_status === 'lost-found') {
		...
		if (!this.myFilesReceiver.length) {
			const message = `Select Image Receiver minimal 1`;
	...
	getComponentTitle() {
		let result = `Edit Package Infrormation`;
		return result;
	}
